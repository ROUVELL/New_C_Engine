...
u8 freelist_should_allocate_one_and_free_one(void) {
    ...
    u64 free_space = freelist_free_space(&list);
    expect_be(total_size - 64, free_space);
...
u8 freelist_should_allocate_one_and_free_multi(void) {
    ...
    expect_true(result);
    expect_be(128, offset3);

    // Verify that the correct amount of space is free.
    u64 free_space = freelist_free_space(&list);
    expect_be(total_size - 192, free_space);
    ...
    free_space = freelist_free_space(&list);
    expect_be(total_size - 128, free_space);
    ...
    // Verify that result is true, offset should be set to the size of the previous allocation.
    expect_true(result);
    expect_be(offset2, offset4); // Offset should be the same as 2 since it occupies the same space
...
u8 freelist_should_allocate_one_and_free_multi_varying_sizes(void) {
    ...
    // Verify that result is true, offset should be set to the offset+size of the previous allocation.
    expect_true(result);
    expect_be(96, offset3);

    // Verify that the correct amount of space is free.
    u64 free_space = freelist_free_space(&list);
    expect_be(total_size - 160, free_space);
    ...
    // Verify that result is true, offset should be set to the size of the previous allocation.
    expect_true(result);
    expect_be(160, offset4); // Offset should be the end since it occupies more space than the old middle block.
...
u8 util_freelist_allocate(freelist* list, alloc_data* data, u64* currently_allocated, u64 total_list_size) {
    ...
    // Verify that result is true
    expect_true(result);
    expect_not_be(data->offset, INVALID_ID);
    ...
    *currently_allocated += data->size;
    ...
    // Verify free space.
    u64 free_space = freelist_free_space(list);
    expect_be(free_space, total_list_size - *currently_allocated);
...
u8 freelist_multiple_alloc_and_free_random(void) {
    ...
    // Pick random sizes.
    const u32 alloc_data_count = 32768;  // 65556
    alloc_data alloc_datas[32768] = {0};
    for (u32 i = 0; i < alloc_data_count; ++i) {
        alloc_datas[i].size = (u64)random_i32_in_range(1, 32768);
    ...
    while (op_count < max_op_count) {
        ...
        if (alloc_count == 0 || random_i32_in_range(0, 99) > 50) {
            while (true) {
                u32 index = (u32)random_i32_in_range(0, alloc_data_count - 1);
                ...
                if (alloc_datas[index].offset == INVALID_ID) {
                    if (!util_freelist_allocate(&list, &alloc_datas[index], &currently_allocated, total_size)) {
                        MERROR("util_allocate failed on index: %u.", index);
                        return false;
                    }
                    alloc_count++;
                    break;
            ...
            while (true) {
                ...
                if (alloc_datas[index].offset != INVALID_ID) {
                    if (!util_freelist_free(&list, &alloc_datas[index], &currently_allocated, total_size)) {
                        MERROR("util_free failed on index: %u.", index);
                        return false;
                    }
                    alloc_count--;
                    break;
