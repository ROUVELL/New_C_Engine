...
u8 hashtable_should_create_and_destroy(void) {
    ...
    hashtable_create(element_size, element_count, false, memory, &table);

    expect_not_be(0, table.memory);
    expect_be(sizeof(u64), table.element_size);
    expect_be(3, table.element_count);

    hashtable_destroy(&table);

    expect_be(0, table.memory);
    expect_be(0, table.element_size);
    expect_be(0, table.element_count);
...
u8 hashtable_should_set_and_get_successfully(void) {
    ...
    expect_be(testval1, get_testval_1);
...
u8 hashtable_should_set_and_get_ptr_successfully(void) {
    ...
    hashtable_create(element_size, element_count, true, memory, &table);

    expect_not_be(0, table.memory);
    expect_be(sizeof(ht_test_struct*), table.element_size);
    expect_be(3, table.element_count);
    ...
    expect_be(testval1->b_value, get_testval_1->b_value);
    expect_be(testval1->u_value, get_testval_1->u_value);
...
u8 hashtable_try_call_non_ptr_on_ptr_table(void) {
    ...
    MDEBUG("The following 2 error messages are intentional.");
...
u8 hashtable_should_set_get_and_update_ptr_successfully(void) {
    ...
    expect_false(get_testval_2->b_value);
    expect_be(99, get_testval_2->u_value);
    expect_float(6.69f, get_testval_2->f_value);
