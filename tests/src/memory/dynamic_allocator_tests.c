...
u8 dynamic_allocator_should_create_and_destroy(void) {
    ...
    void* memory = memory_allocate(memory_requirement, MEMORY_TAG_ENGINE);
    result = dynamic_allocator_create(1024, &memory_requirement, memory, &alloc);
    expect_true(result);
    expect_not_be(0, alloc.memory);
    u64 free_space = dynamic_allocator_free_space(&alloc);
    expect_be(1024, free_space);

    // Destroy the allocator.
    dynamic_allocator_destroy(&alloc);
    expect_be(0, alloc.memory);
    memory_free(memory, memory_requirement, MEMORY_TAG_ENGINE);
    return true;
}
...
u8 dynamic_allocator_single_allocation_all_space(void) {
    ...
    expect_not_be(0, alloc.memory);
    u64 free_space = dynamic_allocator_free_space(&alloc);
    expect_be(total_allocator_size, free_space);

    // Allocate the whole thing.
    void* block = dynamic_allocator_allocate(&alloc, 1024);
    expect_not_be(0, block);

    // Verify free space
    free_space = dynamic_allocator_free_space(&alloc);
    expect_be(0, free_space);

    // Free the allocation
    dynamic_allocator_free(&alloc, block);
...
u8 dynamic_allocator_multi_allocation_all_space(void) {
    ...
    dynamic_allocator_free(&alloc, block3);
    ...
    // Verify free space.
    free_space = dynamic_allocator_free_space(&alloc);
    expect_be(512 + (header_size * 2), free_space);

    // Free the final allocation, out of order
    dynamic_allocator_free(&alloc, block2);
...
u8 dynamic_allocator_multi_allocation_over_allocate(void) {
    ...
    MDEBUG("Note: The following warning and errors are intentionally caused by this test.");
    void* fail_block = dynamic_allocator_allocate(&alloc, 256);
    expect_be(0, fail_block);
...
u8 dynamic_allocator_multi_allocation_most_space_request_too_big(void) {
    ...
    // Verify free space
    free_space = dynamic_allocator_free_space(&alloc);
    expect_be(128 + (header_size * 0), free_space);
...
u8 dynamic_allocator_single_alloc_aligned(void) {
    ...
    expect_true(result);
    expect_be(alignment, block_alignment);
    expect_be(1024, block_size);
...
u8 dynamic_allocator_multiple_alloc_aligned_different_alignments(void) {
    ...
    // Allocate the first one.
    {
        ...
        expect_not_be(0, alloc_datas[idx].block);
        ...
        expect_true(result);
        expect_be(alloc_datas[idx].alignment, block_alignment);
        expect_be(alloc_datas[idx].size, block_size);
        ...
        // Verify free space
        free_space = dynamic_allocator_free_space(&alloc);
        expect_be(total_allocator_size - currently_allocated, free_space);
    }
...
u8 util_allocate(dynamic_allocator* allocator, alloc_data* data, u64* currently_allocated, u64 header_size, u64 total_allocator_size) {
    ...
    expect_true(result);
    expect_be(data->alignment, block_alignment);
    expect_be(data->size, block_size);
...
u8 util_free(dynamic_allocator* allocator, alloc_data* data, u64* currently_allocated, u64 header_size, u64 total_allocator_size) {
    if (!dynamic_allocator_free_aligned(allocator, data->block)) {
        MERROR("util_free, dynamic_allocator_free_aligned failed");
        return false;
    }
    data->block = 0;
    *currently_allocated -= (data->size + header_size + data->alignment);
...
//                 MERROR("util_allocate failed on index: %u.");
//                 return false;
//             }
//             alloc_count++;
//         }
...
//     MTRACE("Randomly allocated %u times. Freeing randomly...", alloc_count);
...
//                 MERROR("util_free failed on index: %u.");
//                 return false;
//             }
//             alloc_count--;
//         }
...
//         total_allocator_size += alloc_datas[i].alignment + header_size + alloc_datas[i].size;
//     }
//     MINFO("Total allocator size: %llu", total_allocator_size);
...
//                 if (alloc_datas[index].block == 0) {
//                     if (!util_allocate(&alloc, &alloc_datas[index], &currently_allocated, header_size, total_allocator_size)) {
//                         MERROR("util_allocate (0) failed on index: %u.", index);
//                         return false;
//                     }
//                     alloc_count++;
//                     break;
...
//                 if (alloc_datas[index].block != 0) {
//                     if (!util_free(&alloc, &alloc_datas[index], &currently_allocated, header_size, total_allocator_size)) {
//                         MERROR("util_free (0) failed on index: %u.", index);
//                         return false;
//                     }
//                     alloc_count--;
//                     break;
...
//     MTRACE("Max op count of %u reached. Freeing remaining allocations.", max_op_count);
//     for (u32 i = 0; i < alloc_data_count; ++i) {
//         if (alloc_datas[i].block != 0) {
//             if (!util_free(&alloc, &alloc_datas[i], &currently_allocated, header_size, total_allocator_size)) {
//                 MERROR("util_free (1) failed on index: %u.");
//                 return false;
//             }
//         }
//     }
